...
var path = require('path'); // 获取验证模块


var authorization = require(path.join(process.cwd(), '/modules/authorization')); // 角色管理模块


var roleServ = authorization.getService('RoleService'); // 获取角色列表
...
router["delete"]('/roles/:id/rights/:rightId', // 参数验证
function (req, res, next) {
  if (!req.params.id) return res.sendResult(null, 400, '角色ID不能为空');
  if (isNaN(parseInt(req.params.id))) res.sendResult(null, 400, '角色ID必须为数字');
